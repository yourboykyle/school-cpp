...
private:
    ...
    // Using a vector so that it's in descending order, just makes searching easier
    ...
    string intToRoman(int value) const {
        if (value > 1000) return "CONCORDIA CUM VERITATE"; // Make sure that results 1000 or less
        string result;
...
public:
    ...
    Roman operator-(const Roman &other) const {
        return Roman(max(0, value - other.value));
    }
    ...
    Roman operator/(const Roman &other) const {
        if (other.value == 0) throw invalid_argument("Division by zero"); // Cannot divide by zero
        return Roman(value / other.value);
    }
